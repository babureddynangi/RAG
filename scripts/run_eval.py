...
SYSTEMS = ["baseline", "metadata", "hybrid", "reranker", "state_rag"]
...
def main():
    ...
    for q in queries:
        ...
        row = {"query_id": q["query_id"], "text": q["text"], "stage": q.get("stage",""), "query_type": q.get("query_type",""), "latency_ms": latency}
        parts = [q["query_id"]]
        for s in SYSTEMS:
            res = results[s]
            ...
            acc[s]["h1"].append(h1); acc[s]["h3"].append(h3); acc[s]["scr"].append(sc); acc[s]["lat"].append(latency)
            parts.append(f"{s}=H1:{h1}/H3:{h3}")
        rows.append(row)
        print(" | ".join(parts))
    ...
    n = len(rows)
    summary = {"n_queries": n, "systems": {}}
    for s in SYSTEMS:
        a = acc[s]
        summary["systems"][s] = {"top1_accuracy": round(sum(a["h1"])/n,4), "top3_accuracy": round(sum(a["h3"])/n,4), "stage_confusion_rate": round(sum(a["scr"])/n,4), "avg_latency_ms": round(sum(a["lat"])/n,1)}
    with open(SUMMARY_PATH, "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\n=== SUMMARY ({n} queries) ===")
    print(f"{'System':<12} {'Top-1':>7} {'Top-3':>7} {'SCR':>7} {'Lat(ms)':>9}")
    print("-"*46)
    for s in SYSTEMS:
        d = summary["systems"][s]
        print(f"{s:<12} {d['top1_accuracy']:>7.4f} {d['top3_accuracy']:>7.4f} {d['stage_confusion_rate']:>7.4f} {d['avg_latency_ms']:>9.1f}")
